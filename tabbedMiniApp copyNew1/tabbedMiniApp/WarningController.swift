...
//    }
    ...
    @IBAction func buttonTapped1(_ sender: UIButton) {
        let fruitIndex = myPicker.selectedRow(inComponent: 0)
        let foodIndex = myPicker.selectedRow(inComponent: 1)
        let sportsIndex = myPicker.selectedRow(inComponent: 2)
        
        let alert = UIAlertController(title: "Your Choices Are", message: "\(componentOne[fruitIndex]) \(componentTwo[foodIndex]) \(componentThree[sportsIndex])", preferredStyle: .alert)
        
        if componentOne[fruitIndex] == componentTwo[foodIndex] && componentOne[fruitIndex] == componentThree[sportsIndex] {
            let okAction = UIAlertAction(title: "👍 Cha ching! That's a Vegas payout!!", style: .default, handler: { action -> Void in})
   ...
   // }
        ...
        let okAction = UIAlertAction(title: "Those are interesting combinations!", style: .default, handler: { action -> Void in})
